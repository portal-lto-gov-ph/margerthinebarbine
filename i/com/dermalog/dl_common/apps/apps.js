...
(function (pSessionTimeoutObserver) {
    ...
    /* TODO:Consider APEX setting "Rejoin Session"
            Habe auÃŸerdem die LOGOUT_URL auf â€žRejoin Session - Enabled for Public Sessionsâ€œ gesetzt.
...
(function (pApps) {
     ...
     * otherwise. The dialog displays using the jQuery UI â€˜Dialogâ€™ widget.
     *
     * There are some differences between this function and a browserâ€™s built-in confirm function:
     ...
     *   the call to apex.message.confirm will run before the user presses OK or Cancel. Therefore acting on the userâ€™s
     *   choice must be done from within the callback, as shown in the example.
     *
     * Note: If either of the following 2 pre-requisites are not met, the function falls back to using the browserâ€™s
     * built-in confirm:
     * - jQuery UI dialog widget code must be loaded on the page.
     * - The browser must be running in â€˜Standardsâ€™ mode. This is because if it is running in â€˜Quirksâ€™ mode (as is the
     ...
     *
     * @example
     * // Displays a confirmation message â€˜Are you sure?â€™, and if OK is pressed executes the â€˜deleteIt()â€™ function.
     ...
     * parameter is called when the dialog is closed. The dialog displays using the jQuery UI â€˜Dialogâ€™ widget.
     *
     * There are some differences between this function and a browserâ€™s built-in alert function:
     * - The dialog style will be consistent with the rest of the app.
     * - The dialog can be moved.
     ...
     *
     * @example
     * // Displays an alert â€˜Load complete.â€™, then after the dialog closes executes the â€˜afterLoad()â€™ function.
